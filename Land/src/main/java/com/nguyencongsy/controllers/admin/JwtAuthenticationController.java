...
//	}
    ...
    public ResponseEntity<?> login(@RequestBody JwtRequest authenticationRequest,HttpServletResponse httpServletResponse)throws Exception{
 		authenticate(authenticationRequest.getUsername(), authenticationRequest.getPassword());
 		final User userDetails = userDetailsService.loadUserByName(authenticationRequest.getUsername());
        ...
        return ResponseEntity.ok(new JwtResponse(token,userDetails.getUsername(),user_role.getRoles(),userDetails.getId()));
    }
